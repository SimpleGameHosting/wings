...
- What: adds `Filesystem.Fingerprint(ctx, ignore)` which walks the server root with the archiver's gitignore matcher and digests `(relative path, size, mtime)` of included files plus the path of included directories and symlinks, with the digest computed over entry lines sorted by relative path after the walk to ensure it is independent of directory enumeration order; exposes it as `POST /api/servers/:server/fingerprint` taking `{"ignore": string}` and returning `{"fingerprint", "files", "duration_ms"}` with a 60s deadline that maps to HTTP 504 with a warning log and request_id.
