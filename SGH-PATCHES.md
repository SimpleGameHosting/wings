...
### router/server: native guided setup apply endpoint

- What: adds `POST /api/servers/:server/setup-apply`, a native job that stops the server (60 second grace, then terminate), applies the guided setup's file changes through the quota-accounted filesystem layer, releases the reservation, and starts the server through the normal power path.
  Adds the `internal/setupapply` package: validated request (`setup_id`, `eula`, `operators`, `whitelist`, `properties`), reads bounded to 1 MiB regular files, merge-by-UUID for `ops.json` and `whitelist.json` preserving unknown fields, a key-preserving `server.properties` patch that keeps comments, order, and line endings, and temp-file plus `Replace` publishing; it never imports package server.
  Admission for both fenced jobs (this one and the modpack install) is now one critical section, `admitFenced` in `server/operation.go`: a repeat of the running or last finished id is answered 202 without a second job, otherwise the reservation and the id are claimed together, so two simultaneous retries can never receive 409 or run twice. The install router previously did the check, the claim, and the record under three separate mutex holds.
  Its counterpart `releaseFenced` releases the reservation only when the id being retired is the one actually holding the fence, since both fence kinds claim the same `OperationInstall` reservation and a stale or duplicated release would otherwise tear the server out from under a job that is still running.
  Publishes `"setup apply status"` (`server/events.go`) with states `stopping, applying, starting, completed, failed` and stable `error_code` values `stop_failed, apply_failed, start_failed, timeout, internal_error`; reports the terminal outcome to the panel through `SendSetupApplyResult`, `POST /api/remote/servers/{uuid}/setup-apply-result`.
  Adds `system.setup_apply.timeout_minutes` (default 5).
  The stop step waits for the environment to actually report itself offline before the start step runs, because `WaitForStop` returns as soon as the process is gone while the transition to the offline state is published a moment later by the attach stream, and a start issued in that window is refused as already running.
